                                    ...
                                    WorkoutPlanExerciseRow(
                                        ...
                                        onUpdate = { sets, repetitions, duration, rest, weight ->
                                            viewModel.updateExerciseSettings(
                                                ...
                                                durationMinutesPerSet = duration,
...
private fun WorkoutPlanExerciseRow(
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            TextField(
                ...
                onValueChange = { value ->
                    ...
                    onUpdate(parsed, exercise.repetitionsPerSet, exercise.durationMinutesPerSet, exercise.restSecondsBetweenSets, exercise.weightKgPerSet)
                },
                modifier = Modifier.weight(1f),
            ...
            TextField(
                ...
                onValueChange = { value ->
                    ...
                    onUpdate(
                        exercise.sets,
                        parsed,
                        if (parsed != null) null else exercise.durationMinutesPerSet,
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            TextField(
                value = exercise.durationMinutesPerSet?.toString().orEmpty(),
                onValueChange = { value ->
                    val cleaned = value.filter { it.isDigit() }
                    val parsed = cleaned.toIntOrNull()
                    onUpdate(
            ...
            TextField(
                value = exercise.restSecondsBetweenSets?.toString().orEmpty(),
                onValueChange = { value ->
                    val cleaned = value.filter { it.isDigit() }
                    val parsed = cleaned.toIntOrNull()
                    onUpdate(exercise.sets, exercise.repetitionsPerSet, exercise.durationMinutesPerSet, parsed, exercise.weightKgPerSet)
                },
                modifier = Modifier.weight(1f),
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            TextField(
                ...
                onValueChange = { value ->
                    ...
                    onUpdate(exercise.sets, exercise.repetitionsPerSet, exercise.durationMinutesPerSet, exercise.restSecondsBetweenSets, parsed)
                },
                modifier = Modifier.weight(1f),
...
private fun exerciseSettingsSummary(exercise: WorkoutPlanExercise): String? {
    val segments = buildList {
        ...
        exercise.durationMinutesPerSet?.let { add("$it min/set") }
        exercise.restSecondsBetweenSets?.let { add("$it sec rest") }
